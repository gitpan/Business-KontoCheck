...
#define VERSION "2.92 (3.0 Beta 2)"
#define VERSION_DATE "2008-08-23"
         ...
         case LUT2_2_BLZ:
            ...
            if(!(blz=calloc(lut2_cnt_hs+1,sizeof(int)))
               ...
               for(i=0,eptr=data+len;ptr<eptr && i<lut2_cnt_hs;i++){
                  startidx[i]=i; /* fixme: startidx ansehen  +§§§1 */
                  j=UI *ptr++;
                  switch(j){
                     case 254:
                        C2UI(j,ptr);
            ...
            release_data=1;
             if(!(filialen=calloc(len,sizeof(int)))
               ...
               for(i=j=0,ptr=data,eptr=data+len;i<len;i++){
                  startidx[i]+=j; /* fixme: startidx ansehen  +§§§1 */
 ...
 */
      ...
      case 2074:
         if(retvals){
            ...
            if(pz==5)pz=0;  /* fixme: fraglich, ob in dem Fall 0 oder 5 genommen wird */
            CHECK_PZ10M5;
         }
         else
            return FALSE;
...
DLL_EXPORT int rebuild_blzfile(char *inputname,char *outputname,UINT4 set)
{
   ...
   */
   ...
   ret=generate_lut2(inputname,tmpfile,"Testdatei für LUT2",NULL,lut_set,20,3,set);
...
DLL_EXPORT int get_lut_id(char *lut_name,int set,char *id){return EXCLUDED_AT_COMPILETIME;}
...
DLL_EXPORT int lut_init(char *lut_name,UINT4 required,UINT4 set){return EXCLUDED_AT_COMPILETIME;}
...
DLL_EXPORT int iban_check(char *iban,int *retval){return EXCLUDED_AT_COMPILETIME;}
