...
static const char *(*retval_enc)(int)=NULL;
...
{
   char *ptr,*zptr,*dptr,*name_hauptstelle=NULL,*name_start=NULL,*data,**sortc_buf1,**sortc_buf2;
   ...
   if(set==2)typ+=SET_OFFSET;  /* sekundäres Set schreiben */
       ...
       * Buffer data für die Blockdaten (Rohwerte).
       */
   iptr=(int*)out_buffer;
   sortc_buf1=(char**)iptr+sizeof(int)*(bank_cnt+10);
   sortc_buf2=(char**)sortc_buf1+sizeof(char*)*(bank_cnt+10);
   sorti_buf=(int*)sortc_buf1;
   data=(char*)sortc_buf2+sizeof(char*)*(bank_cnt+10);
   switch(typ){
      ...
      case LUT2_2_FILIALEN:
         ...
         /* out_buffer ist der Speicherplatz für die Ausgabe; die Ausgabe nicht
          * direkt bei outbuffer beginnnen lassen, sondern Platz für Array (char**,int*) lassen
          * erst dann die Ausgabe beginnen. Im Array die Sortierelemente speichern und im zweiten
          * Schritt dann sortieren und schreiben.
          */
      ...
      case LUT2_2_NAME:
         ...
         CHECK_RETURN(write_lut_block_int(lut,typ,dptr-data,data));
            ...
            /* Index-Block für Namen sortieren und schreiben */
         if(add_idx){
            ...
            CHECK_RETURN(write_lut_block_int(lut,LUT2_NAME_SORT+(typ-LUT2_NAME),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_NAME_NAME_KURZ:
         ...
         if(add_idx){
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_NAME_SORT+(typ-LUT2_NAME_NAME_KURZ),dptr-data,data));

               /* Index-Block für Kurznamen sortieren und schreiben */
            sortc_buf=sortc_buf2;
            for(i=0;i<j;i++)iptr[i]=i;   /* Index-Array initialisieren */
            qsort(iptr,j,sizeof(int),qcmp_sortc);
            dptr=data;
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_NAME_KURZ_SORT+(typ-LUT2_NAME_NAME_KURZ),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_PLZ:
         ...
         CHECK_RETURN(write_lut_block_int(lut,typ,dptr-data,data));
            ...
            /* Index-Block für PLZ generieren und schreiben */
         if(add_idx){
            for(i=0;i<j;i++)iptr[i]=i;   /* Index-Array initialisieren */
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_PLZ_SORT+(typ-LUT2_PLZ),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_ORT:
         ...
         CHECK_RETURN(write_lut_block_int(lut,typ,dptr-data,data));
            ...
            /* Index-Block für Ort sortieren und schreiben */
         if(add_idx){
            sortc_buf=sortc_buf1;
            for(i=0;i<j;i++)iptr[i]=i;   /* Index-Array initialisieren */
            qsort(iptr,j,sizeof(int),qcmp_sortc);
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_ORT_SORT+(typ-LUT2_ORT),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_NAME_KURZ:
         ...
         if(add_idx){
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_NAME_KURZ_SORT+(typ-LUT2_NAME_KURZ),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_BIC:
         ...
         CHECK_RETURN(write_lut_block_int(lut,typ,dptr-data,data));
            ...
            /* Index-Block für BIC generieren und schreiben */
         if(add_idx){
            for(i=j=0,dptr=data;i<bank_cnt;i++)if(auch_filialen || qs_hauptstelle[qs_sortidx[i]]=='1'){
               zptr=qs_zeilen[qs_sortidx[i]];
               sortc_buf1[j++]=dptr;
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_BIC_SORT+(typ-LUT2_BIC),dptr-data,data));
         }
         break;
      ...
      case LUT2_2_PZ:
         ...
         if(add_idx){
            ...
            UI2C(j,dptr);
            for(i=0;i<j;i++)UI2C(iptr[i],dptr);
            CHECK_RETURN(write_lut_block_int(lut,LUT2_PZ_SORT+(typ-LUT2_PZ),dptr-data,data));
         }
         break;
 ...
 * #                                                                         #
 2 # Für den Parameter mode werden die folgenden Werte akzeptiert:           #
 * #     1,'i','I':  ISO-8859-1                                              #
 * #     2,'u','U':  UTF-8                                                   #
 * #     3,'h','H':  HTML-Entities                                           #
 * #     4,'d','D':  DOS (CP850)                                             #
